...
//making the market place contract
...
contract CustomDex {
    ...
    // mappping to maintain the tokens and its instances
    mapping(string => ERC20) public tokenInstanceMap;

    uint256 ethValue = 100000000000000;


    //a struct to store all tokens, address and valur
    struct History {
        uint256 historyId;
        string tokenA;
        string tokenB;
    ...
    //creating a map of that struct
    uint256 public _historyIndex;
    mapping(uint256 => History) private historys;


    //Create a new CustomToken object
    constructor() {
        for (uint i=0; i<tokens.length; i++){
            CustomToken token = new CustomToken(tokens[i], tokens[i]);//passing token name and token symbol
    ...
    //this function will update the information in the script when any swap will happen 
    ...
    //function to swap the token from native blockchain currency to any erc20 token
    ...
    function swapTokenToEth(string memory tokenName, uint256 _amount) public returns (uint256){
        //Convert the token ammount (ethValue) to exact amount (10)
